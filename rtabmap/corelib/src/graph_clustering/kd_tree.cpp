...
namespace graph_clustering
{
        ...
        bool exclude_pt)
    {
        ...
        KDNodePtr further = nearest_(section, pt, next_lv, best_l, best_dist_l, exclude_pt);
        if (!further->x.empty())
        {
            double dl = dist2(further->x, pt);
            if (dl < best_dist_l)
            {
                if ((dl == 0 && !exclude_pt) || dl != 0)
                {
                    best_dist_l = dl;
                    best_l = further;
                }
        ...
        if (dx2 < best_dist_l)
        {
            further = nearest_(other, pt, next_lv, best_l, best_dist_l, exclude_pt);
            if (!further->x.empty())
            {
                double dl = dist2(further->x, pt);
    ...
    // default caller
    KDNodePtr KDTree::nearest_(const point_t &pt, bool exclude_pt)
    {
        size_t level = 0;
    ...
    point_t KDTree::nearest_point(const point_t &pt, bool exclude_pt)
    {
        return point_t(*nearest_(pt, exclude_pt));
    };
    size_t KDTree::nearest_index(const point_t &pt, bool exclude_pt)
    {
        return size_t(*nearest_(pt, exclude_pt));
    };

    pointIndex KDTree::nearest_pointIndex(const point_t &pt, bool exclude_pt)
    {
        KDNodePtr Nearest = nearest_(pt, exclude_pt);
        return pointIndex(point_t(*Nearest), size_t(*Nearest));
    }

    bool KDTree::contains(const point_t &pt)
    {
        return (dist2(pt, nearest_point(pt, false)) == 0);
    }
